...
final class Money private (val amount: BigDecimal)(val currency: Currency)
  ...
  override def toString =
    amount
      .setScale(currency.formatDecimals, BigDecimal.RoundingMode.HALF_EVEN)
  ...
  def toFormattedString =
    currency.symbol + amount.setScale(currency.formatDecimals).toString
   ...
   */
  def moneyPlus(that: Money)(implicit context: MoneyContext =
                               defaultMoneyContext) = context.add(this, that)

  /** moneyPlus **/
  def +(that: Money)(implicit context: MoneyContext = defaultMoneyContext) =
  ...
  override def plus(that: Money): Money = that.currency match {
    ...
    case _ =>
      throw new UnsupportedOperationException(
          "plus not supported for cross-currency comparison - use moneyPlus")
  }
   ...
   * @throws NoSuchExchangeRateException when no exchange rate is available
   */
  def moneyMinus(that: Money)(implicit context: MoneyContext =
                                defaultMoneyContext) =
    context.subtract(this, that)
  ...
  override def minus(that: Money): Money = that.currency match {
    ...
    case _ =>
      throw new UnsupportedOperationException(
          "minus not supported for cross-currency comparison - use moneyMinus")
  }
  ...
  def moneyDivide(that: Money)(implicit context: MoneyContext): BigDecimal =
    context.divide(this, that)
  def /(that: Money)(
      implicit context: MoneyContext = defaultMoneyContext): BigDecimal =
    moneyDivide(that)
  ...
  override def divide(that: Money): Double = that.currency match {
    ...
    case _ =>
      throw new UnsupportedOperationException(
          "divide not supported for cross-currency comparison - use moneyDivide")
  }
  ...
  override def max(that: Money): Money = (that, that.currency) match {
    ...
    case _ =>
      throw new UnsupportedOperationException(
          "max not supported for cross-currency comparison - use moneyMax")
  }
  ...
  override def min(that: Money): Money = (that, that.currency) match {
    ...
    case _ =>
      throw new UnsupportedOperationException(
          "min not supported for cross-currency comparison - use moneyMin")
  }
  ...
  override def compare(that: Money): Int = that.currency match {
    case this.currency =>
      if (this.amount > that.amount) 1
      else if (this.amount < that.amount) -1
      else 0
    case _ =>
      throw new UnsupportedOperationException(
          "Comparison between Moneys of dislike Currency is not supported")
  }
   ...
   * @return
   */
  def moneyMax(that: Money)(implicit moneyContext: MoneyContext) =
    moneyContext.compare(this, that) match {
      case -1 => that
      case _ => this
    }
   ...
   * @return
   */
  def moneyMin(that: Money)(implicit moneyContext: MoneyContext) =
    moneyContext.compare(this, that) match {
      case 1 => that
      case _ => this
    }
   ...
   * @return
   */
  def moneyEquals(that: Money)(implicit moneyContext: MoneyContext) =
    moneyCompare(that) == 0
   ...
   * @return
   */
  def moneyNotEquals(that: Money)(implicit moneyContext: MoneyContext) =
    moneyCompare(that) != 0
   ...
   * @return
   */
  def moneyCompare(that: Money)(implicit moneyContext: MoneyContext) =
    moneyContext.compare(this, that)
  ...
  def toThe(that: Money) = that.currency match {
    case this.currency =>
      throw new IllegalArgumentException(
          "Can not create Exchange Rate on matching currencies")
   ...
   * @throws NoSuchExchangeRateException when no exchange rate is available
   */
  def to(unit: Currency)(implicit context: MoneyContext) =
    context.convert(this, unit).amount
   ...
   * @throws NoSuchExchangeRateException when no exchange rate is available
   */
  def in(unit: Currency)(implicit context: MoneyContext) =
    context.convert(this, unit)
}
...
object Money extends Dimension[Money] {
  def apply(value: Double)(implicit fxContext: MoneyContext) =
    new Money(BigDecimal(value))(fxContext.defaultCurrency)
  def apply(value: BigDecimal)(implicit fxContext: MoneyContext) =
    new Money(value)(fxContext.defaultCurrency)

  def apply(value: BigDecimal, currency: Currency) = new Money(value)(currency)
  def apply(value: BigDecimal, currency: String) =
    new Money(value)(defaultCurrencyMap(currency))

  def apply[A](n: A, currency: Currency)(implicit num: Numeric[A]) =
    new Money(BigDecimal(num.toDouble(n)))(currency)
  def apply[A](n: A, currency: String)(implicit num: Numeric[A]) =
    new Money(BigDecimal(num.toDouble(n)))(defaultCurrencyMap(currency))

  def apply(s: String): Try[Money] = {
    lazy val regex = ("([-+]?[0-9]*\\.?[0-9]+) *(" + defaultCurrencySet
          .map(_.code)
          .reduceLeft(_ + "|" + _) + ")").r
    s match {
      case regex(value, currency) =>
        Success(Money(value.toDouble, defaultCurrencyMap(currency)))
  ...
  def primaryUnit =
    ??? // Should not be used with Money - drawn from MoneyContext instead
  def siUnit =
    ??? // Should not be used with Money - drawn from MoneyContext instead
  def units =
    ??? // Should not be used with Money - drawn from MoneyContext instead
...
abstract class Currency(val code: String,
    ...
    extends UnitOfMeasure[Money] {
  def apply[A](n: A)(implicit num: Numeric[A]) =
    Money(BigDecimal(num.toDouble(n)), this)
...
object MoneyConversions {
  ...
  implicit def fromLong(l: Long): MoneyConversions[BigDecimal] =
    new MoneyConversions(BigDecimal(l))
  implicit def fromDouble(d: Double): MoneyConversions[BigDecimal] =
    new MoneyConversions(BigDecimal(d))

  implicit class MoneyConversions[A](n: A)(implicit num: Numeric[A]) {
    def money(implicit context: MoneyContext) =
      Money(n, context.defaultCurrency)
  ...
  class MoneyNumeric()(implicit mc: MoneyContext) extends Numeric[Money] {
    ...
    def times(x: Money, y: Money) =
      throw new UnsupportedOperationException(
          "Numeric.times not supported for Quantities")
