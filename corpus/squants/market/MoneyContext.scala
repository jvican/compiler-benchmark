 ...
 */
case class MoneyContext(defaultCurrency: Currency,
   ...
   * @return
   */
  def directRateFor(curA: Currency,
                    curB: Currency): Option[CurrencyExchangeRate] = {
    rates.find(
        r =>
   ...
   * @return
   */
  def indirectRateFor(curA: Currency,
                      curB: Currency): Option[CurrencyExchangeRate] = {
    ...
    directRateFor(curA, curB) match {
      ...
      case _ =>
        val ratesWithCurA = rates.filter(r =>
              r.base.currency == curA || r.counter.currency == curA)
        val ratesWithCurB = rates.filter(r =>
              r.base.currency == curB || r.counter.currency == curB)

        val curs = for {
          cur <- currencies
          if ratesWithCurA.map(_.base.currency).contains(cur) || ratesWithCurA
            .map(_.counter.currency)
            .contains(cur)
          if ratesWithCurB.map(_.base.currency).contains(cur) || ratesWithCurB
            .map(_.counter.currency)
            .contains(cur)
        } yield cur
        ...
        curs.headOption match {
          case Some(cur) =>
            Some(
                CurrencyExchangeRate(convert(cur(1), curA),
                                     convert(cur(1), curB)))
          case None => None
        }
    }
  ...
  def convert(money: Money, currency: Currency): Money = {
    ...
    if (money.currency == currency) money
      ...
      directRateFor(money.currency, currency) match {
          ...
          indirectRateFor(money.currency, currency) match {
            ...
            case None =>
              throw new NoSuchExchangeRateException(
                  s"Rate for currency pair (${money.currency} / $currency)")
          }
        case _ =>
          throw new NoSuchExchangeRateException(
              s"Rate for currency pair (${money.currency} / $currency)")
  ...
  def add(moneyA: Money, moneyB: Money): Money =
    moneyA.currency(moneyA.amount + convert(moneyB, moneyA.currency).amount)
  ...
  def divide(moneyA: Money, moneyB: Money): BigDecimal =
    moneyA.amount / convert(moneyB, moneyA.currency).amount
   ...
   * @return
   */
  def withExchangeRates(rates: List[CurrencyExchangeRate]) =
    copy(rates = rates)
}
