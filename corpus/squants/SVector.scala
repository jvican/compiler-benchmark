...
trait SVector[A] {
   ...
   * @return Angle
   */
  def angle(coordinateX: Int = 0,
            coordinateY: Int = 1,
            unit: AngleUnit = Radians): Angle
   ...
   * @return (A, Angle)
   */
  def polar(coordinateX: Int = 0,
            coordinateY: Int = 1,
            unit: AngleUnit = Radians): (A, Angle) =
...
object SVector {
  ...
  def apply[A <: Quantity[A]](coordinates: A*): QuantityVector[A] =
    QuantityVector[A](coordinates: _*)
  ...
  def apply(radius: Double, theta: Angle): DoubleVector =
    apply(radius * theta.cos, radius * theta.sin)
   ...
   * @return
   */
  def apply[A <: Quantity[A]](radius: A, theta: Angle): QuantityVector[A] =
    apply(radius * theta.cos, radius * theta.sin)
}
...
case class DoubleVector(coordinates: Double*) extends SVector[Double] {
  ...
  def magnitude: Double =
    math.sqrt(coordinates.toTraversable.map(v => v * v).sum)
  def angle(coordinateX: Int = 0,
            coordinateY: Int = 1,
            unit: AngleUnit = Radians): Angle =
  ...
  def map[A <: Double](f: Double => Double): DoubleVector =
    DoubleVector(coordinates.toTraversable.map(f).toSeq: _*)
  ...
  def map[A <: Quantity[A]](f: Double => A): QuantityVector[A] =
    QuantityVector(coordinates.toTraversable.map(f).toSeq: _*)

  def plus(that: SVectorType): SVectorType =
    DoubleVector(
        coordinates.toIterable
          .zipAll(that.coordinates.toIterable, 0d, 0d)
          .toTraversable
          .map(v => v._1 + v._2)
          .toSeq: _*)
  ...
  def dotProduct[B <: Quantity[B]](that: QuantityVector[B]) =
    that dotProduct this

  def crossProduct(that: SVectorType) =
    (this.coordinates.length, that.coordinates.length) match {
      case (3, 3) =>
        DoubleVector(coordinates(1) * that
                       .coordinates(2) - coordinates(2) * that.coordinates(1),
                     coordinates(2) * that
                       .coordinates(0) - coordinates(0) * that.coordinates(2),
                     coordinates(0) * that
                       .coordinates(1) - coordinates(1) * that.coordinates(0))
      case (7, 7) =>
        throw new UnsupportedOperationException(
            "Seven-dimensional cross product is not currently supported")
      case _ =>
        throw new UnsupportedOperationException(
            "Cross product is not supported on vectors with an arbitrary number of dimensions")
    }

  def crossProduct[B <: Quantity[B]](that: QuantityVector[B]) =
 ...
 */
case class QuantityVector[A <: Quantity[A]](coordinates: A*)
    extends SVector[A] {
  ...
  def magnitude: A =
    valueUnit(
        math.sqrt(coordinates.toTraversable
              .map(v => v.to(valueUnit) * v.to(valueUnit))
              .sum))
  def angle(coordinateX: Int = 0,
            coordinateY: Int = 1,
            unit: AngleUnit = Radians): Angle =
  ...
  def plus(that: SVectorType): SVectorType =
    QuantityVector(
        coordinates.toIterable
          .zipAll(that.coordinates.toIterable, valueUnit(0), valueUnit(0))
          .toTraversable
          .map(v => v._1 + v._2)
          .toSeq: _*)
  def minus(that: SVectorType): SVectorType =
  ...
  def times[B <: Quantity[B], C <: Quantity[C]](
      quantTimes: A => C): QuantityVector[C] = map(quantTimes)
  ...
  def divide[B <: Quantity[B], C <: Quantity[C]](
      quantDiv: A => C): QuantityVector[C] = map(quantDiv(_))

  def dotProduct(that: DoubleVector): A =
    valueUnit(
        coordinates.toIterable
          .zipAll(that.coordinates.toIterable, valueUnit(0), 0d)
          .toTraversable
          .map(v => v._1.to(valueUnit) * v._2)
          .sum)

  def dotProduct[B <: Quantity[B], C <: Quantity[C]](
      that: SVector[B],
      quantTimes: (A, B) => C)(implicit num: Numeric[C]): C =
    coordinates.toIterable
      .zipAll(that.coordinates.toIterable,
              valueUnit(0),
              that.coordinates.head.unit(0))
      .toTraversable
      .map(v => quantTimes(v._1, v._2))
      .sum
  ...
  def crossProduct(that: DoubleVector): SVectorType =
    (this.coordinates.length, that.coordinates.length) match {
      case (3, 3) =>
        QuantityVector(
            (coordinates(1) * that.coordinates(2)) - (coordinates(2) * that
                  .coordinates(1)),
            (coordinates(2) * that.coordinates(0)) - (coordinates(0) * that
                  .coordinates(2)),
            (coordinates(0) * that.coordinates(1)) - (coordinates(1) * that
                  .coordinates(0)))
      case (7, 7) =>
        throw new UnsupportedOperationException(
            "Seven-dimensional Cross Product is not currently supported")
  ...
  def crossProduct[B <: Quantity[B], C <: Quantity[C]](
      that: SVector[B],
      quantTimes: (A, B) => C)(implicit num: Numeric[C]): QuantityVector[C] = {
    (this.coordinates.length, that.coordinates.length) match {
      case (3, 3) =>
        QuantityVector(
            quantTimes(coordinates(1), that.coordinates(2)) - quantTimes(
                coordinates(2),
                that.coordinates(1)),
            quantTimes(coordinates(2), that.coordinates(0)) - quantTimes(
                coordinates(0),
                that.coordinates(2)),
            quantTimes(coordinates(0), that.coordinates(1)) - quantTimes(
                coordinates(1),
                that.coordinates(0)))
      case (7, 7) =>
        throw new UnsupportedOperationException(
