...
final class SolidAngle private (val value: Double, val unit: SolidAngleUnit)
  ...
  def *(that: LuminousIntensity): LuminousFlux =
    Lumens(toSquaredRadians * that.toCandelas)
  def *(that: RadiantIntensity): Power =
    Watts(toSquaredRadians * that.toWattsPerSteradian)
...
object SolidAngle extends Dimension[SolidAngle] {
  private[space] def apply[A](n: A, unit: SolidAngleUnit)(
      implicit num: Numeric[A]) = new SolidAngle(num.toDouble(n), unit)
  def apply = parse _
  def name = "SolidAngle"
...
object SolidAngleConversions {
  ...
  implicit object SolidAngleNumeric
      extends AbstractQuantityNumeric[SolidAngle](SolidAngle.primaryUnit)
}
