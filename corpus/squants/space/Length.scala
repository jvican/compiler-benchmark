...
final class Length private (val value: Double, val unit: LengthUnit)
  ...
  def *(that: SpectralIntensity): RadiantIntensity =
    WattsPerSteradian(toMeters * that.toWattsPerSteradianPerMeter)
  def *(that: SpectralPower): Power = Watts(toMeters * that.toWattsPerMeter)
  def *(that: Conductivity): ElectricalConductance =
    Siemens(toMeters * that.toSiemensPerMeter)
  def *(that: ElectricalResistance): Resistivity =
    OhmMeters(toMeters * that.toOhms)
...
object Length extends Dimension[Length] with BaseDimension {
  private[space] def apply[A](n: A, unit: LengthUnit)(
      implicit num: Numeric[A]) = new Length(num.toDouble(n), unit)
  def apply = parse _
  def name = "Length"
...
object LengthConversions {
  ...
  implicit object LengthNumeric
      extends AbstractQuantityNumeric[Length](Length.primaryUnit)
}
