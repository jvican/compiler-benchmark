...
final class Volume private (val value: Double, val unit: VolumeUnit)
  ...
  def *(that: Density): Mass =
    Kilograms(toCubicMeters * that.toKilogramsPerCubicMeter)
  def *(that: EnergyDensity): Energy =
    Joules(toCubicMeters * that.toJoulesPerCubicMeter)
...
object Volume extends Dimension[Volume] {
  private[space] def apply[A](n: A, unit: VolumeUnit)(
      implicit num: Numeric[A]) = new Volume(num.toDouble(n), unit)
  def apply = parse _
  def name = "Volume"
...
object VolumeConversions {
  ...
  implicit object VolumeNumeric
      extends AbstractQuantityNumeric[Volume](Volume.primaryUnit)
}
