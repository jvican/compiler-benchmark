...
object Angle extends Dimension[Angle] {
  private[space] def apply[A](n: A, unit: AngleUnit)(
      implicit num: Numeric[A]) = new Angle(num.toDouble(n), unit)
  def apply = parse _
  def name = "Angle"
...
object AngleConversions {
  ...
  implicit object AngleNumeric
      extends AbstractQuantityNumeric[Angle](Angle.primaryUnit)
}
