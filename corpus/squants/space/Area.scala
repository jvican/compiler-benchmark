...
final class Area private (val value: Double, val unit: AreaUnit)
  ...
  def *(that: AreaDensity): Mass =
    Kilograms(toSquareMeters * that.toKilogramsPerSquareMeter)
  ...
  def *(that: Luminance): LuminousIntensity =
    Candelas(toSquareMeters * that.toCandelasPerSquareMeters)
  def *(that: MagneticFluxDensity): MagneticFlux =
    Webers(toSquareMeters * that.toTeslas)
  def *(that: Irradiance): Power =
    Watts(toSquareMeters * that.toWattsPerSquareMeter)
  def *(that: Radiance): RadiantIntensity =
    WattsPerSteradian(toSquareMeters * that.toWattsPerSteradianPerSquareMeter)
...
object Area extends Dimension[Area] {
  private[space] def apply[A](n: A, unit: AreaUnit)(implicit num: Numeric[A]) =
    new Area(num.toDouble(n), unit)
  def apply = parse _
  def name = "Area"
...
object AreaConversions {
  ...
  implicit object AreaNumeric
      extends AbstractQuantityNumeric[Area](Area.primaryUnit)
}
