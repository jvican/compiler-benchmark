 ...
 */
final class EnergyDensity private (val value: Double,
                                   val unit: EnergyDensityUnit)
    extends Quantity[EnergyDensity] {

  def dimension = EnergyDensity

  def *(that: Volume): Energy =
    Joules(toJoulesPerCubicMeter * that.toCubicMeters)

  def toJoulesPerCubicMeter = to(JoulesPerCubicMeter)
}

object EnergyDensity extends Dimension[EnergyDensity] {
  private[energy] def apply[A](n: A, unit: EnergyDensityUnit)(
      implicit num: Numeric[A]) = new EnergyDensity(num.toDouble(n), unit)
  def apply = parse _
