...
final class Power private (val value: Double, val unit: PowerUnit)
  ...
  def /(that: Area): Irradiance =
    WattsPerSquareMeter(toWatts / that.toSquareMeters)
  def /(that: Irradiance): Area =
    SquareMeters(this.toWatts / that.toWattsPerSquareMeter)
  def /(that: RadiantIntensity): SolidAngle =
    SquaredRadians(toWatts / that.toWattsPerSteradian)
  def /(that: SolidAngle): RadiantIntensity =
    WattsPerSteradian(toWatts / that.toSteradians)
  def /(that: ElectricPotential): ElectricCurrent =
    Amperes(toWatts / that.toVolts)
  def /(that: ElectricCurrent): ElectricPotential =
    Volts(toWatts / that.toAmperes)
...
object Power extends Dimension[Power] {
  private[energy] def apply[A](n: A, unit: PowerUnit)(
      implicit num: Numeric[A]) = new Power(num.toDouble(n), unit)
  def apply(energy: Energy, time: Time): Power =
    apply(energy.toWattHours / time.toHours, Watts)
  def apply = parse _
...
object PowerConversions {
  ...
  implicit object PowerNumeric
      extends AbstractQuantityNumeric[Power](Power.primaryUnit)
}
