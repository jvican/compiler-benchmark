 ...
 */
final class SpecificEnergy private (val value: Double,
                                    val unit: SpecificEnergyUnit)
    extends Quantity[SpecificEnergy] {
...
object SpecificEnergy extends Dimension[SpecificEnergy] {
  private[energy] def apply[A](n: A, unit: SpecificEnergyUnit)(
      implicit num: Numeric[A]) = new SpecificEnergy(num.toDouble(n), unit)
  def apply = parse _
...
trait SpecificEnergyUnit
    extends UnitOfMeasure[SpecificEnergy]
    with UnitConverter {
...
object SpecificEnergyConversions {
  ...
  implicit object SpecificEnergyNumeric
      extends AbstractQuantityNumeric[SpecificEnergy](
          SpecificEnergy.primaryUnit)
}
