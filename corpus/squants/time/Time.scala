...
final class Time private (val value: Double, val unit: TimeUnit)
  ...
  def *[A <: squants.Quantity[A] & squants.time.TimeIntegral[_]](
      that: TimeDerivative[A]): A = that * this
...
object Time extends Dimension[Time] with BaseDimension {
  ...
  private[time] def apply[A](n: A, unit: TimeUnit)(implicit num: Numeric[A]) =
    new Time(num.toDouble(n), unit)
  def apply = parse _
...
object TimeConversions {
  ...
  implicit object TimeNumeric
      extends AbstractQuantityNumeric[Time](Time.primaryUnit)
