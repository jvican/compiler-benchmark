...
final class Mass private (val value: Double, val unit: MassUnit)
  ...
  def *(that: Acceleration): Force =
    Newtons(toKilograms * that.toMetersPerSecondSquared)
  def /(that: Density): Volume =
    CubicMeters(toKilograms / that.toKilogramsPerCubicMeter)
  def /(that: Volume): Density = Density(this, that)
  def /(that: AreaDensity): Area =
    SquareMeters(toKilograms / that.toKilogramsPerSquareMeter)
  def /(that: Area): AreaDensity =
    KilogramsPerSquareMeter(toKilograms / that.toSquareMeters)
...
object MassConversions {
  ...
  implicit object MassNumeric
      extends AbstractQuantityNumeric[Mass](Mass.primaryUnit)
}
