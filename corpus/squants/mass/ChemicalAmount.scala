 ...
 */
final class ChemicalAmount private (val value: Double,
                                    val unit: ChemicalAmountUnit)
...
object ChemicalAmount extends Dimension[ChemicalAmount] with BaseDimension {
  private[mass] def apply[A](n: A, unit: ChemicalAmountUnit)(
      implicit num: Numeric[A]) = new ChemicalAmount(num.toDouble(n), unit)
  def apply = parse _
...
trait ChemicalAmountUnit
    extends UnitOfMeasure[ChemicalAmount]
    with UnitConverter {
...
object ChemicalAmountConversions {
  ...
  implicit object ChemicalAmountNumeric
      extends AbstractQuantityNumeric[ChemicalAmount](
          ChemicalAmount.primaryUnit)
}
