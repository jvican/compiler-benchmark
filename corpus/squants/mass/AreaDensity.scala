...
final class AreaDensity private (val value: Double, val unit: AreaDensityUnit)
  ...
  def *(that: Area): Mass =
    Kilograms(toKilogramsPerSquareMeter * that.toSquareMeters)
...
object AreaDensity extends Dimension[AreaDensity] {
  private[mass] def apply[A](n: A, unit: AreaDensityUnit)(
      implicit num: Numeric[A]) = new AreaDensity(num.toDouble(n), unit)
  def apply(mass: Mass, area: Area): AreaDensity =
    KilogramsPerSquareMeter(mass.toKilograms / area.toSquareMeters)
  def apply = parse _
  def name = "AreaDensity"
  def primaryUnit = KilogramsPerSquareMeter
  def siUnit = KilogramsPerSquareMeter
  def units =
    Set(KilogramsPerSquareMeter, KilogramsPerHectare, GramsPerSquareCentimeter)
}
...
object KilogramsPerSquareMeter
    extends AreaDensityUnit
    with PrimaryUnit
    with SiUnit {
  val symbol = "kg/m²"
}
...
object AreaDensityConversions {
  ...
  implicit object AreaDensityNumeric
      extends AbstractQuantityNumeric[AreaDensity](AreaDensity.primaryUnit)
}
