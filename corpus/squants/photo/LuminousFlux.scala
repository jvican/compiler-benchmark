...
final class LuminousFlux private (val value: Double,
  ...
  def /(that: SolidAngle): LuminousIntensity =
    Candelas(toLumens / that.toSquaredRadians)
  def /(that: LuminousIntensity): SolidAngle =
    SquaredRadians(toLumens / that.toCandelas)

  def toLumens = to(Lumens)
}

object LuminousFlux extends Dimension[LuminousFlux] {
  private[photo] def apply[A](n: A, unit: LuminousFluxUnit)(
      implicit num: Numeric[A]) = new LuminousFlux(num.toDouble(n), unit)
  def apply = parse _
...
object LuminousFluxConversions {
  ...
  implicit object LuminousFluxNumeric
      extends AbstractQuantityNumeric[LuminousFlux](LuminousFlux.primaryUnit)
}
