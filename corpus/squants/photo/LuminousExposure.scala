 ...
 */
final class LuminousExposure private (val value: Double,
                                      val unit: LuminousExposureUnit)
...
object LuminousExposure extends Dimension[LuminousExposure] {
  private[photo] def apply[A](n: A, unit: LuminousExposureUnit)(
      implicit num: Numeric[A]) = new LuminousExposure(num.toDouble(n), unit)
  def apply = parse _
...
trait LuminousExposureUnit
    extends UnitOfMeasure[LuminousExposure]
    with UnitConverter {
...
object LuminousExposureConversions {
  ...
  implicit object LuminousExposureNumeric
      extends AbstractQuantityNumeric[LuminousExposure](
          LuminousExposure.primaryUnit)
}
