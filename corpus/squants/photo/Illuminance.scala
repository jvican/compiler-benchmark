...
object Illuminance extends Dimension[Illuminance] {
  private[photo] def apply[A](n: A, unit: IlluminanceUnit)(
      implicit num: Numeric[A]) = new Illuminance(num.toDouble(n), unit)
  def apply = parse _
...
object IlluminanceConversions {
  ...
  implicit object IlluminanceNumeric
      extends AbstractQuantityNumeric[Illuminance](Illuminance.primaryUnit)
}
