...
object Luminance extends Dimension[Luminance] {
  private[photo] def apply[A](n: A, unit: LuminanceUnit)(
      implicit num: Numeric[A]) = new Luminance(num.toDouble(n), unit)
  def apply = parse _
...
trait LuminanceUnit extends UnitOfMeasure[Luminance] {
  def apply[A](n: A)(implicit num: Numeric[A]) =
    Luminance(num.toDouble(n), this)
}

object CandelasPerSquareMeter
...
object LuminanceConversions {
  ...
  implicit object LuminanceNumeric
      extends AbstractQuantityNumeric[Luminance](Luminance.primaryUnit)
}
