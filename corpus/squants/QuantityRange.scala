...
case class QuantityRange[A <: Quantity[A]](lower: A, upper: A) {
  if (lower >= upper)
    throw new IllegalArgumentException(
        "QuantityRange upper bound must be greater than or equal to the lower bound")
  ...
  def times(multiple: Double): QuantitySeries[A] = {
    ...
    val ranges = (0 until count).map(n =>
          QuantityRange(lower + (toQuantity * n), upper + (toQuantity * n)))
    if (remainder > 0)
      ranges :+ QuantityRange(lower + (toQuantity * count),
  ...
  def divide(that: A): QuantitySeries[A] = {
    ...
    def accumulate(acc: QuantitySeries[A], start: A): QuantitySeries[A] = {
      if (start >= upper) acc
        ...
        accumulate(acc :+ (start to (start + that).min(upper)), start + that)
    }
  ...
  /** foldLeft */
  def /:[B](size: A, z: B)(op: (B, QuantityRange[A]) => B) =
    foldLeft(size, z)(op)
  ...
  /** foldLeft */
  def /:[B](divisor: Double, z: B)(op: (B, QuantityRange[A]) => B) =
    foldLeft(divisor, z)(op)
  ...
  /** foldRight */
  def :\[B](size: A, z: B)(op: (QuantityRange[A], B) => B) =
    foldRight(size, z)(op)
  ...
  /** foldRight */
  def :\[B](divisor: Double, z: B)(op: (QuantityRange[A], B) => B) =
    foldRight(divisor, z)(op)
   ...
   * @return
   */
  def partiallyContains(range: QuantityRange[A]) =
   ...
   * @return
   */
  def partiallyIncludes(range: QuantityRange[A]) =
    range.lower <= upper && range.upper >= lower
