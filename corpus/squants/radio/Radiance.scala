...
final class Radiance private (val value: Double, val unit: RadianceUnit)
  ...
  def *(that: Area): RadiantIntensity =
    WattsPerSteradian(toWattsPerSteradianPerSquareMeter * that.toSquareMeters)
  def /(that: RadiantIntensity): Area =
    SquareMeters(toWattsPerSteradianPerSquareMeter / that.toWattsPerSteradian)

  def toWattsPerSteradianPerSquareMeter = to(WattsPerSteradianPerSquareMeter)
}

object Radiance extends Dimension[Radiance] {
  private[radio] def apply[A](n: A, unit: RadianceUnit)(
      implicit num: Numeric[A]) = new Radiance(num.toDouble(n), unit)
  def apply = parse _
  def name = "Radiance"
...
object WattsPerSteradianPerSquareMeter
...
object RadianceConversions {
  ...
  implicit object RadianceNumeric
      extends AbstractQuantityNumeric[Radiance](Radiance.primaryUnit)
}
