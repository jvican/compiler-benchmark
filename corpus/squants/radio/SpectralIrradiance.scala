 ...
 * @param value Double
 */
final class SpectralIrradiance private (val value: Double,
                                        val unit: SpectralIrradianceUnit)
...
object SpectralIrradiance extends Dimension[SpectralIrradiance] {
  private[radio] def apply[A](n: A, unit: SpectralIrradianceUnit)(
      implicit num: Numeric[A]) = new SpectralIrradiance(num.toDouble(n), unit)
  def apply = parse _
  ...
  def units =
    Set(WattsPerCubicMeter,
        WattsPerSquareMeterPerMicron,
        WattsPerSquareMeterPerNanometer,
        ErgsPerSecondPerSquareCentimeterPerAngstrom)
}

trait SpectralIrradianceUnit
    extends UnitOfMeasure[SpectralIrradiance]
    with UnitConverter {
  def apply[A](n: A)(implicit num: Numeric[A]) = SpectralIrradiance(n, this)
}

object WattsPerCubicMeter
...
object SpectralIrradianceConversions {
  ...
  implicit class SpectralIrradianceConversions[A](n: A)(
    ...
    def ergsPerSecondPerSquareCentimeterPerAngstrom =
      ErgsPerSecondPerSquareCentimeterPerAngstrom(n)
  }

  implicit object SpectralIrradianceNumeric
      extends AbstractQuantityNumeric[SpectralIrradiance](
          SpectralIrradiance.primaryUnit)
}
