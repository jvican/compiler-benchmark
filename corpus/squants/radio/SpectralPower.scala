 ...
 * @param value Double
 */
final class SpectralPower private (val value: Double,
                                   val unit: SpectralPowerUnit)
...
object SpectralPower extends Dimension[SpectralPower] {
  private[radio] def apply[A](n: A, unit: SpectralPowerUnit)(
      implicit num: Numeric[A]) = new SpectralPower(num.toDouble(n), unit)
  def apply = parse _
...
object SpectralPowerConversions {
  ...
  implicit object SpectralPowerNumeric
      extends AbstractQuantityNumeric[SpectralPower](SpectralPower.primaryUnit)
}
