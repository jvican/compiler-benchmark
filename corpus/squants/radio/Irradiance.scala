...
object Irradiance extends Dimension[Irradiance] {
  private[radio] def apply[A](n: A, unit: IrradianceUnit)(
      implicit num: Numeric[A]) = new Irradiance(num.toDouble(n), unit)
  def apply = parse _
  def name = "Irradiance"
...
object IrradianceConversions {
  ...
  lazy val ergsPerSecondPerSquareCentimeter = ErgsPerSecondPerSquareCentimeter(
      1)
