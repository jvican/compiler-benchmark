 ...
 */
final class ThermalCapacity private (val value: Double,
                                     val unit: ThermalCapacityUnit)
    extends Quantity[ThermalCapacity] {

  def dimension = ThermalCapacity

  def *(that: Temperature): Energy =
    Joules(toJoulesPerKelvin * that.toKelvinScale)

  def toJoulesPerKelvin = to(JoulesPerKelvin)
}

object ThermalCapacity extends Dimension[ThermalCapacity] {
  private[thermal] def apply[A](n: A, unit: ThermalCapacityUnit)(
      implicit num: Numeric[A]) = new ThermalCapacity(num.toDouble(n), unit)
  def apply = parse _
...
trait ThermalCapacityUnit
    extends UnitOfMeasure[ThermalCapacity]
    with UnitConverter {
  def apply[A](n: A)(implicit num: Numeric[A]) = ThermalCapacity(n, this)
}

object JoulesPerKelvin
    extends ThermalCapacityUnit
    with PrimaryUnit
    with SiUnit {
  def symbol = "J/K"
}
...
object ThermalCapacityConversions {
  ...
  implicit object ThermalCapacityNumeric
      extends AbstractQuantityNumeric[ThermalCapacity](
          ThermalCapacity.primaryUnit)
}
