...
final class Temperature private (val value: Double, val unit: TemperatureScale)
  ...
  override def plus(that: Temperature): Temperature =
    Temperature(value + that.convert(unit, withOffset = false).value, unit)
  ...
  private def convert(toScale: TemperatureScale,
                      withOffset: Boolean = true): Temperature =
    (unit, toScale, withOffset) match {
      case (Fahrenheit, Fahrenheit, _) => this
      case (Celsius, Celsius, _) => this
      case (Kelvin, Kelvin, _) => this

      case (Fahrenheit, Celsius, true) =>
        Celsius(TemperatureConversions.fahrenheitToCelsiusScale(value))
      case (Celsius, Fahrenheit, true) =>
        Fahrenheit(TemperatureConversions.celsiusToFahrenheitScale(value))
      case (Celsius, Kelvin, true) =>
        Kelvin(TemperatureConversions.celsiusToKelvinScale(value))
      case (Kelvin, Celsius, true) =>
        Celsius(TemperatureConversions.kelvinToCelsiusScale(value))
      case (Fahrenheit, Kelvin, true) =>
        Kelvin(TemperatureConversions.fahrenheitToKelvinScale(value))
      case (Kelvin, Fahrenheit, true) =>
        Fahrenheit(TemperatureConversions.kelvinToFahrenheitScale(value))

      case (Fahrenheit, Celsius, false) =>
        Celsius(TemperatureConversions.fahrenheitToCelsiusDegrees(value))
      case (Celsius, Fahrenheit, false) =>
        Fahrenheit(TemperatureConversions.celsiusToFahrenheitDegrees(value))
      case (Celsius, Kelvin, false) =>
        Kelvin(TemperatureConversions.celsiusToKelvinDegrees(value))
      case (Kelvin, Celsius, false) =>
        Celsius(TemperatureConversions.kelvinToCelsiusDegrees(value))
      case (Fahrenheit, Kelvin, false) =>
        Kelvin(TemperatureConversions.fahrenheitToKelvinDegrees(value))
      case (Kelvin, Fahrenheit, false) =>
        Fahrenheit(TemperatureConversions.kelvinToFahrenheitDegrees(value))
    }
  ...
  def toDegrees(unit: TemperatureScale) =
...
object Temperature extends Dimension[Temperature] with BaseDimension {
  def apply[A](n: A, scale: TemperatureScale)(implicit num: Numeric[A]) =
    new Temperature(num.toDouble(n), scale)

  def apply(s: String): Try[Temperature] = {
    val regex = "([-+]?[0-9]*\\.?[0-9]+)[ °]*(f|F|c|C|k|K)".r
    s match {
      case regex(value, Fahrenheit.symbol) =>
        Success(Fahrenheit(value.toDouble))
      ...
      case regex(value, "K") => Success(Kelvin(value.toDouble))
      case _ =>
        Failure(QuantityParseException("Unable to parse Temperature", s))
    }
  }
...
sealed trait TemperatureScale extends UnitOfMeasure[Temperature] {
  ...
  def apply[A](n: A)(implicit num: Numeric[A]) =
    Temperature(num.toDouble(n), this)
}
...
object TemperatureConversions {
  ...
  def fahrenheitToCelsiusScale(fahrenheit: Double) =
    (fahrenheit - 32d) * 5d / 9d
  def celsiusToKelvinScale(celsius: Double) = celsius + 273.15
  def kelvinToCelsiusScale(kelvin: Double) = kelvin - 273.15
  def fahrenheitToKelvinScale(fahrenheit: Double) =
    (fahrenheit + 459.67) * 5d / 9d
  ...
  implicit class TemperatureConversions[A](n: A)(implicit num: Numeric[A]) {
    ...
    def F = Fahrenheit(n)
    def Fah =
      Fahrenheit(n) // F conflicts with (Float) in the console; Fah is provided as an alternative
