...
final class Acceleration private (val value: Double,
  ...
  protected[squants] def timeIntegrated =
    MetersPerSecond(toMetersPerSecondSquared)
  protected[squants] def timeDerived =
    MetersPerSecondCubed(toMetersPerSecondSquared)
  protected[squants] def time = Seconds(1)

  def *(that: Mass): Force =
    Newtons(toMetersPerSecondSquared * that.toKilograms)
...
object Acceleration extends Dimension[Acceleration] {
  private[motion] def apply[A](n: A, unit: AccelerationUnit)(
      implicit num: Numeric[A]) = new Acceleration(num.toDouble(n), unit)
  def apply = parse _
  def name = "Acceleration"
  def primaryUnit = MetersPerSecondSquared
  def siUnit = MetersPerSecondSquared
  def units =
    Set(FeetPerSecondSquared,
        MetersPerSecondSquared,
        UsMilesPerHourSquared,
        EarthGravities)
}
...
object UsMilesPerHourSquared extends AccelerationUnit {
  val symbol = "mph²"
  val conversionFactor = (UsMiles.conversionFactor * Meters.conversionFactor) / math
      .pow(Time.SecondsPerHour, 2)
}
...
object AccelerationConversions {
  ...
  implicit object AccelerationNumeric
      extends AbstractQuantityNumeric[Acceleration](Acceleration.primaryUnit)
}
