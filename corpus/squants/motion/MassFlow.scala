...
object MassFlow extends Dimension[MassFlow] {
  private[motion] def apply[A](n: A, unit: MassFlowUnit)(
      implicit num: Numeric[A]) = new MassFlow(num.toDouble(n), unit)
  def apply = parse _
  def name = "MassFlow"
  def primaryUnit = KilogramsPerSecond
  def siUnit = KilogramsPerSecond
  def units =
    Set(KilogramsPerSecond,
        PoundsPerSecond,
        PoundsPerHour,
        KilopoundsPerHour,
        MegapoundsPerHour)
...
object MassFlowConversions {
  ...
  implicit object MassFlowNumeric
      extends AbstractQuantityNumeric[MassFlow](MassFlow.primaryUnit)
}
