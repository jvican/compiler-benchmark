 ...
 *
 */
final class AngularVelocity private (val value: Double,
...
object AngularVelocity extends Dimension[AngularVelocity] {
  private[motion] def apply[A](n: A, unit: AngularVelocityUnit)(
      implicit num: Numeric[A]) = new AngularVelocity(num.toDouble(n), unit)
  def apply = parse _
  def name = "AngularVelocity"
  def primaryUnit = RadiansPerSecond
  def siUnit = RadiansPerSecond
  def units =
    Set(RadiansPerSecond, DegreesPerSecond, GradsPerSecond, TurnsPerSecond)
}

trait AngularVelocityUnit
    extends UnitOfMeasure[AngularVelocity]
    with UnitConverter {
  def apply[A](n: A)(implicit num: Numeric[A]) = AngularVelocity(n, this)
}

object RadiansPerSecond
...
object AngularVelocityConversions {
  ...
  implicit object AngularVelocityNumeric
      extends AbstractQuantityNumeric[AngularVelocity](
          AngularVelocity.primaryUnit)
}
