 ...
 * @param value Double
 */
final class VolumeFlow private (val value: Double,
                                val unit: VolumeFlowRateUnit)
...
object VolumeFlow extends Dimension[VolumeFlow] {
  private[motion] def apply[A](n: A, unit: VolumeFlowRateUnit)(
      implicit num: Numeric[A]) = new VolumeFlow(num.toDouble(n), unit)
  def apply = parse _
  def name = "VolumeFlow"
  def primaryUnit = CubicMetersPerSecond
  def siUnit = CubicMetersPerSecond
  def units =
    Set(CubicMetersPerSecond,
        CubicFeetPerHour,
        GallonsPerDay,
        GallonsPerHour,
        GallonsPerMinute,
...
object CubicMetersPerSecond
    extends VolumeFlowRateUnit
    with PrimaryUnit
    with SiUnit {
  val symbol = "m³/s"
}
...
object VolumeFlowConversions {
  ...
  implicit object VolumeFlowNumeric
      extends AbstractQuantityNumeric[VolumeFlow](CubicMetersPerSecond)
}
