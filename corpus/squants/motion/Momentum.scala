...
final class Momentum private (val value: Double, val unit: MomentumUnit)
  ...
  def /(that: Velocity): Mass =
    Kilograms(toNewtonSeconds / that.toMetersPerSecond)
  def /(that: Mass): Velocity =
    MetersPerSecond(toNewtonSeconds / that.toKilograms)
...
object Momentum extends Dimension[Momentum] {
  private[motion] def apply[A](n: A, unit: MomentumUnit)(
      implicit num: Numeric[A]) = new Momentum(num.toDouble(n), unit)
  def apply(m: Mass, v: Velocity): Momentum =
    NewtonSeconds(m.toKilograms * v.toMetersPerSecond)
  def apply = parse _
  def name = "Momentum"
...
object MomentumConversions {
  ...
  implicit object MomentumNumeric
      extends AbstractQuantityNumeric[Momentum](Momentum.primaryUnit)
}
