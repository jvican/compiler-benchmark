...
final class Force private (val value: Double, val unit: ForceUnit)
    ...
    with TimeDerivative[Momentum]
    with TimeIntegral[Yank] {
  ...
  def /(that: Mass): Acceleration =
    MetersPerSecondSquared(toNewtons / that.toKilograms)
  def /(that: Acceleration): Mass =
    Kilograms(toNewtons / that.toMetersPerSecondSquared)
...
object Force extends Dimension[Force] {
  private[motion] def apply[A](n: A, unit: ForceUnit)(
      implicit num: Numeric[A]) = new Force(num.toDouble(n), unit)
  def apply = parse _
  def name = "Force"
...
object ForceConversions {
  ...
  implicit object ForceNumeric
      extends AbstractQuantityNumeric[Force](Force.primaryUnit)
}
