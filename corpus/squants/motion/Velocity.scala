...
final class Velocity private (val value: Double, val unit: VelocityUnit)
  ...
  def *(that: Mass): Momentum =
    NewtonSeconds(toMetersPerSecond * that.toKilograms)
...
object Velocity extends Dimension[Velocity] {
  private[motion] def apply[A](n: A, unit: VelocityUnit)(
      implicit num: Numeric[A]) = new Velocity(num.toDouble(n), unit)
...
object KilometersPerHour extends VelocityUnit {
  val symbol = "km/h"
  val conversionFactor = (Kilometers.conversionFactor / Meters.conversionFactor) / squants.time.Time.SecondsPerHour
}

object UsMilesPerHour extends VelocityUnit {
  val symbol = "mph"
  val conversionFactor = (UsMiles.conversionFactor * Meters.conversionFactor) / squants.time.Time.SecondsPerHour
}

object InternationalMilesPerHour extends VelocityUnit {
  val symbol = "imph"
  val conversionFactor = (InternationalMiles.conversionFactor / Meters.conversionFactor) / squants.time.Time.SecondsPerHour
}

object Knots extends VelocityUnit {
  val symbol = "kn"
  val conversionFactor = (NauticalMiles.conversionFactor / Meters.conversionFactor) / squants.time.Time.SecondsPerHour
}
...
object VelocityConversions {
  ...
  implicit object VelocityNumeric
      extends AbstractQuantityNumeric[Velocity](Velocity.primaryUnit)
}
