...
object Pressure extends Dimension[Pressure] {
  private[motion] def apply[A](n: A, unit: PressureUnit)(
      implicit num: Numeric[A]) = new Pressure(num.toDouble(n), unit)
  def apply = parse _
  def name = "Pressure"
...
object PressureConversions {
  ...
  implicit object PressureNumeric
      extends AbstractQuantityNumeric[Pressure](Pressure.primaryUnit)
}
