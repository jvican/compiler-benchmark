 ...
 * @param value Double
 */
final class PressureChange private (val value: Double,
                                    val unit: PressureChangeUnit)
...
object PressureChange extends Dimension[PressureChange] {
  private[motion] def apply[A](n: A, unit: PressureChangeUnit)(
      implicit num: Numeric[A]) = new PressureChange(num.toDouble(n), unit)
  def apply = parse _
  def name = "PressureChange"
  def primaryUnit = PascalsPerSecond
  def siUnit = PascalsPerSecond
  def units =
    Set(PascalsPerSecond,
        BarsPerSecond,
        PoundsPerSquareInchPerSecond,
        StandardAtmospheresPerSecond)
}

trait PressureChangeUnit
    extends UnitOfMeasure[PressureChange]
    with UnitConverter {
  def apply[A](n: A)(implicit num: Numeric[A]) = PressureChange(n, this)
}

object PascalsPerSecond
    extends PressureChangeUnit
    with PrimaryUnit
    with SiUnit {
  val symbol = "Pa/s"
}
...
object PressureChangeConversions {
  ...
  implicit object PressureChangeNumeric
      extends AbstractQuantityNumeric[PressureChange](
          PressureChange.primaryUnit)
}
