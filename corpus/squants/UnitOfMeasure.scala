...
trait UnitOfMeasure[A <: Quantity[A]] extends Serializable {
   ...
   * @return
   */
  final def convertTo[N](n: N)(implicit num: Numeric[N]) =
    converterTo(num.toDouble(n))
   ...
   * @return
   */
  final def convertFrom[N](n: N)(implicit num: Numeric[N]) =
    converterFrom(num.toDouble(n))
}
