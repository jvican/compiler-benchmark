...
trait Dimension[A <: Quantity[A]] {
  ...
  private def parseTuple(value: Double, symbol: String): Try[A] = {
    symbolToUnit(symbol) match {
      ...
      case None =>
        Failure(
            QuantityParseException(s"Unable to identify $name unit $symbol",
                                   (value, symbol).toString()))
    }
  }
}

case class QuantityParseException(message: String, expression: String)
    extends Exception
