...
abstract class Quantity[A <: Quantity[A]]
  ...
  def divideAndRemainder(that: A): (Double, A) =
    BigDecimal(value) /% that.to(unit) match {
   ...
   */
  def compare(that: A) =
    if (value > that.to(unit)) 1 else if (value < that.to(unit)) -1 else 0
  ...
  def plusOrMinus(that: A): QuantityRange[A] =
    QuantityRange(this - that, this + that)
  ...
  def toString(uom: UnitOfMeasure[A]): String =
    s"${crossFormat(to(uom))} ${uom.symbol}"
  ...
  def toString(uom: UnitOfMeasure[A], format: String): String =
    "%s %s".format(format.format(to(uom)), uom.symbol)
