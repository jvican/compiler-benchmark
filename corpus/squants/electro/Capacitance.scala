...
final class Capacitance private (val value: Double, val unit: CapacitanceUnit)
  ...
  def *(that: ElectricPotential): ElectricCharge =
    Coulombs(this.toFarads * that.toVolts)
...
object Capacitance extends Dimension[Capacitance] {
  private[electro] def apply[A](n: A, unit: CapacitanceUnit)(
      implicit num: Numeric[A]) = new Capacitance(num.toDouble(n), unit)
  def apply = parse _
  def name = "Capacitance"
  def primaryUnit = Farads
  def siUnit = Farads
  def units =
    Set(Farads, Picofarads, Nanofarads, Microfarads, Millifarads, Kilofarads)
}
...
object CapacitanceConversions {
  ...
  implicit object CapacitanceNumeric
      extends AbstractQuantityNumeric[Capacitance](Capacitance.primaryUnit)
}
