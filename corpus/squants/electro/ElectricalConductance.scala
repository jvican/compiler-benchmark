...
final class ElectricalConductance private (val value: Double,
                                           val unit: ElectricalConductanceUnit)
    extends Quantity[ElectricalConductance] {

  def dimension = ElectricalConductance

  def /(that: Length): Conductivity =
    SiemensPerMeter(toSiemens / that.toMeters)
  def /(that: Conductivity): Length =
    Meters(toSiemens / that.toSiemensPerMeter)
...
object ElectricalConductance extends Dimension[ElectricalConductance] {
  private[electro] def apply[A](n: A, unit: ElectricalConductanceUnit)(
      implicit num: Numeric[A]) =
    new ElectricalConductance(num.toDouble(n), unit)
  def apply = parse _
...
trait ElectricalConductanceUnit
    extends UnitOfMeasure[ElectricalConductance]
    with UnitConverter {
...
object ElectricalConductanceConversions {
  ...
  implicit class ElectricalConductanceConversions[A](n: A)(
      implicit num: Numeric[A]) {
    def siemens = Siemens(n)
  }

  implicit object ElectricalConductanceNumeric
      extends AbstractQuantityNumeric[ElectricalConductance](
          ElectricalConductance.primaryUnit)
}
