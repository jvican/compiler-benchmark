...
final class ElectricCharge private (val value: Double,
  ...
  def *(that: ElectricPotential): Energy =
    Joules(this.toCoulombs * that.toVolts)
  def /(that: ElectricPotential): Capacitance =
    Farads(toCoulombs / that.toVolts)
  def /(that: Capacitance): ElectricPotential =
    Volts(toCoulombs / that.toFarads)
...
object ElectricCharge extends Dimension[ElectricCharge] {
  private[electro] def apply[A](n: A, unit: ElectricChargeUnit)(
      implicit num: Numeric[A]) = new ElectricCharge(num.toDouble(n), unit)
  def apply = parse _
...
trait ElectricChargeUnit
    extends UnitOfMeasure[ElectricCharge]
    with UnitConverter {
...
object ElectricChargeConversions {
  ...
  implicit object ElectricalChargeNumeric
      extends AbstractQuantityNumeric[ElectricCharge](
          ElectricCharge.primaryUnit)
}
