 ...
 */
final class MagneticFlux private (val value: Double,
                                  val unit: MagneticFluxUnit)
  ...
  def /(that: Area): MagneticFluxDensity =
    Teslas(toWebers / that.toSquareMeters)
  def /(that: MagneticFluxDensity): Area =
    SquareMeters(toWebers / that.toTeslas)
...
object MagneticFlux extends Dimension[MagneticFlux] {
  private[electro] def apply[A](n: A, unit: MagneticFluxUnit)(
      implicit num: Numeric[A]) = new MagneticFlux(num.toDouble(n), unit)
  def apply = parse _
...
object MagneticFluxConversions {
  ...
  implicit object MagneticFluxNumeric
      extends AbstractQuantityNumeric[MagneticFlux](MagneticFlux.primaryUnit)
}
