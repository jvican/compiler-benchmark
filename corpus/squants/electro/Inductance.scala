...
object Inductance extends Dimension[Inductance] {
  private[electro] def apply[A](n: A, unit: InductanceUnit)(
      implicit num: Numeric[A]) = new Inductance(num.toDouble(n), unit)
  def apply = parse _
  def name = "Inductance"
...
object InductanceConversions {
  ...
  implicit object InductanceNumeric
      extends AbstractQuantityNumeric[Inductance](Inductance.primaryUnit)
}
