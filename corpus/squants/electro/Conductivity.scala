 ...
 */
final class Conductivity private (val value: Double,
                                  val unit: ConductivityUnit)
    extends Quantity[Conductivity] {

  def dimension = Conductivity

  def *(that: Length): ElectricalConductance =
    Siemens(toSiemensPerMeter * that.toMeters)
...
object Conductivity extends Dimension[Conductivity] {
  private[electro] def apply[A](n: A, unit: ConductivityUnit)(
      implicit num: Numeric[A]) = new Conductivity(num.toDouble(n), unit)
  def apply = parse _
...
object ConductivityConversions {
  ...
  implicit object ConductivityNumeric
      extends AbstractQuantityNumeric[Conductivity](Conductivity.primaryUnit)
}
