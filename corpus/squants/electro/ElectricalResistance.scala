...
object ElectricalResistance extends Dimension[ElectricalResistance] {
  private[electro] def apply[A](n: A, unit: ElectricalResistanceUnit)(
      implicit num: Numeric[A]) =
    new ElectricalResistance(num.toDouble(n), unit)
  def apply = parse _
  def name = "ElectricalResistance"
  def primaryUnit = Ohms
  def siUnit = Ohms
  def units =
    Set(Ohms, Nanohms, Microohms, Milliohms, Kilohms, Megohms, Gigohms)
}

trait ElectricalResistanceUnit
    extends UnitOfMeasure[ElectricalResistance]
    with UnitConverter {
...
object ElectricalResistanceConversions {
  ...
  implicit class ElectricalResistanceConversions[A](n: A)(
      implicit num: Numeric[A]) {
    def ohms = Ohms(n)
  ...
  implicit object ElectricalResistanceNumeric
      extends AbstractQuantityNumeric[ElectricalResistance](
          ElectricalResistance.primaryUnit)
}
