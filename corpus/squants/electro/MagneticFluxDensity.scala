 ...
 */
final class MagneticFluxDensity private (val value: Double,
                                         val unit: MagneticFluxDensityUnit)
...
object MagneticFluxDensity extends Dimension[MagneticFluxDensity] {
  private[electro] def apply[A](n: A, unit: MagneticFluxDensityUnit)(
      implicit num: Numeric[A]) =
    new MagneticFluxDensity(num.toDouble(n), unit)
  def apply = parse _
...
trait MagneticFluxDensityUnit
    extends UnitOfMeasure[MagneticFluxDensity]
    with UnitConverter {
...
object MagneticFluxDensityConversions {
  ...
  implicit class MagneticFluxDensistyConversions[A](n: A)(
      implicit num: Numeric[A]) {
    def teslas = Teslas(n)
    def gauss = Gauss(n)
  }
  ...
  implicit object MagneticFluxDensistyNumeric
      extends AbstractQuantityNumeric[MagneticFluxDensity](
          MagneticFluxDensity.primaryUnit)
}
