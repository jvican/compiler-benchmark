 ...
 */
final class ElectricPotential private (val value: Double,
                                       val unit: ElectricPotentialUnit)
  ...
  def /(that: ElectricCurrent): ElectricalResistance =
    Ohms(toVolts / that.toAmperes)
  def /(that: ElectricalResistance): ElectricCurrent =
    Amperes(toVolts / that.toOhms)
...
object ElectricPotential extends Dimension[ElectricPotential] {
  private[electro] def apply[A](n: A, unit: ElectricPotentialUnit)(
      implicit num: Numeric[A]) = new ElectricPotential(num.toDouble(n), unit)
  def apply = parse _
...
trait ElectricPotentialUnit
    extends UnitOfMeasure[ElectricPotential]
    with UnitConverter {
...
object ElectricPotentialConversions {
  ...
  implicit class ElectricPotentialConversions[A](n: A)(
  ...
  implicit object ElectricPotentialNumeric
      extends AbstractQuantityNumeric[ElectricPotential](
          ElectricPotential.primaryUnit)
}
