 ...
 */
final class ElectricCurrent private (val value: Double,
                                     val unit: ElectricCurrentUnit)
  ...
  def *(that: ElectricalResistance): ElectricPotential =
    Volts(toAmperes * that.toOhms)
  ...
  def /(that: ElectricPotential): ElectricalConductance =
    Siemens(toAmperes / that.toVolts)
...
object ElectricCurrent extends Dimension[ElectricCurrent] with BaseDimension {
  private[electro] def apply[A](n: A, unit: ElectricCurrentUnit)(
      implicit num: Numeric[A]) = new ElectricCurrent(num.toDouble(n), unit)
  def apply = parse _
 ...
 */
trait ElectricCurrentUnit
    extends UnitOfMeasure[ElectricCurrent]
    with UnitConverter {
...
object ElectricCurrentConversions {
  ...
  implicit object ElectricCurrentNumeric
      extends AbstractQuantityNumeric[ElectricCurrent](
          ElectricCurrent.primaryUnit)
}
